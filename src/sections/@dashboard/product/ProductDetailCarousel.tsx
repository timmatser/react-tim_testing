import Slider from 'react-slick';
import { useCallback, useEffect, useRef, useState } from 'react';
import { sortBy } from 'lodash';
// @mui
import { alpha, styled } from '@mui/material/styles';
import { Box } from '@mui/material';
// @types
import { ProductImages } from '../../../@types/products';
// utils
import axios from '../../../utils/axios';
// components
import Image from '../../../components/Image';
import LightboxModal from '../../../components/LightboxModal';
import { CarouselArrowIndex } from '../../../components/carousel';
// config
import { API_LANG } from '../../../config';
// redux
import { dispatch } from '../../../redux/store';
import { setFetching } from '../../../redux/slices/api';

// ----------------------------------------------------------------------

const THUMB_SIZE = 64;

const RootStyle = styled('div')(({ theme }) => ({
  '& .slick-slide': {
    float: theme.direction === 'rtl' ? 'right' : 'left',
    '&:focus': { outline: 'none' },
  },
}));

// ----------------------------------------------------------------------

type Props = {
  id: string | undefined;
};

export default function ProductDetailCarousel({ id }: Props) {
  const [openLightbox, setOpenLightbox] = useState(false);

  const [selectedImage, setSelectedImage] = useState<number>(0);

  const [currentIndex, setCurrentIndex] = useState(0);

  const [nav1, setNav1] = useState<Slider>();

  const [nav2, setNav2] = useState<Slider>();

  const slider1 = useRef<Slider | null>(null);

  const slider2 = useRef<Slider | null>(null);
  const [productImage, setImages] = useState<ProductImages[]>([]);
  const getImages = useCallback(async () => {
    try {
      dispatch(setFetching(true))
      const response = await axios.get(`/${API_LANG}/products/${id}/images.json`);
      setImages(sortBy(response.data.productImages, ['sortOrder']));
    } catch (error) {
      console.error(error);
    } finally {
      dispatch(setFetching(false))
    }
  }, [id]);

  useEffect(() => {
    getImages().then();
  }, [getImages]);

  const imagesLightbox = productImage.map((_image) => _image.src);

  const handleOpenLightbox = (url: string) => {
    const selectedImage = imagesLightbox.findIndex((index) => index === url);
    setOpenLightbox(true);
    setSelectedImage(selectedImage);
  };

  const settings1 = {
    dots: false,
    arrows: false,
    slidesToShow: 1,
    draggable: false,
    slidesToScroll: 1,
    adaptiveHeight: true,
    beforeChange: (current: number, next: number) => {
      setCurrentIndex(next);
    },
  };

  const settings2 = {
    dots: false,
    arrows: false,
    centerMode: true,
    swipeToSlide: true,
    focusOnSelect: true,
    variableWidth: true,
    centerPadding: '0px',
    slidesToShow: imagesLightbox?.length > 3 ? 3 : imagesLightbox?.length,
  };

  useEffect(() => {
    if (slider1.current) {
      setNav1(slider1.current);
    }
    if (slider2.current) {
      setNav2(slider2.current);
    }
  }, []);

  const handlePrevious = () => {
    slider2.current?.slickPrev();
  };

  const handleNext = () => {
    slider2.current?.slickNext();
  };

  return (
    <RootStyle>
      {
        <Box sx={{ p: 1 }}>
          <Box sx={{ zIndex: 0, borderRadius: 2, overflow: 'hidden', position: 'relative' }}>
            <Slider {...settings1} asNavFor={nav2} ref={slider1}>
              {productImage?.map((img, index) => (
                <Box key={index} sx={{ px: 0.75 }}>
                  <Image
                    alt="large image"
                    src={img?.src}
                    ratio="1/1"
                    onClick={() => handleOpenLightbox(img?.src)}
                    sx={{ cursor: 'zoom-in' }}
                  />
                </Box>
              ))}
            </Slider>
            {productImage.length > 1 && (
              <CarouselArrowIndex
                index={currentIndex}
                total={productImage.length}
                onNext={handleNext}
                onPrevious={handlePrevious}
              />
            )}
          </Box>
        </Box>
      }
      {productImage.length > 1 && (
        <Box
          sx={{
            my: 3,
            mx: 'auto',
            '& .slick-current .isActive': { opacity: 1 },
            ...(imagesLightbox.length === 1 && { maxWidth: THUMB_SIZE + 16 }),
            ...(imagesLightbox.length === 2 && { maxWidth: THUMB_SIZE * 2 + 32 }),
            ...(imagesLightbox.length === 3 && { maxWidth: THUMB_SIZE * 3 + 48 }),
            ...(imagesLightbox.length === 4 && { maxWidth: THUMB_SIZE * 3 + 48 }),
            ...(imagesLightbox.length >= 5 && { maxWidth: THUMB_SIZE * 5 }),
            ...(imagesLightbox.length > 2 && {
              position: 'relative',
              '&:before, &:after': {
                top: 0,
                zIndex: 9,
                content: "''",
                height: '100%',
                position: 'absolute',
                width: THUMB_SIZE / 3,
                backgroundImage: (theme) =>
                  `linear-gradient(to left, ${alpha(theme.palette.background.paper, 0)} 0%, ${
                    theme.palette.background.paper
                  } 100%)`,
              },
              '&:after': { right: 0, transform: 'scaleX(-1)' },
            }),
          }}
        >
          <Slider {...settings2} asNavFor={nav1} ref={slider2}>
            {productImage.map((img, index) => (
              <Box key={index} sx={{ px: 0.75 }}>
                <Image
                  disabledEffect
                  alt="thumb image"
                  src={img?.src}
                  sx={{
                    width: THUMB_SIZE,
                    height: THUMB_SIZE,
                    borderRadius: 1.5,
                    cursor: 'pointer',
                    ...(currentIndex === index && {
                      border: (theme) => `solid 3px ${theme.palette.primary.main}`,
                    }),
                  }}
                />
              </Box>
            ))}
          </Slider>
        </Box>
      )}

      <LightboxModal
        images={imagesLightbox}
        mainSrc={imagesLightbox[selectedImage]}
        photoIndex={selectedImage}
        setPhotoIndex={setSelectedImage}
        isOpen={openLightbox}
        onCloseRequest={() => setOpenLightbox(false)}
      />
    </RootStyle>
  );
}
